...
function JoyTeleopPanel(props: JoyTeleopPanelProps): JSX.Element {
  ...
  // Wipe latched toggles whenever sticky mode is switched off. Without this,
  // re-enabling sticky instantly republishes toggles latched in a previous
  // sticky session, and a pad swapped onto the same Gamepad.index while
  // sticky was off (prune only runs while enabled) inherits the old state.
  useEffect(() => {
    if (!config.stickyButtons) {
      sticky.reset();
    }
  }, [config.stickyButtons, sticky]);
  ...
  const applyArmedOverlay = useCallback(
    (base: GamepadSnapshot | undefined, forceMerge = false): GamepadSnapshot | undefined => {
      ...
      // Visualizer-only: light up the deadman slot so arming is visible in the
      // mimic. The publish path synthesizes the deadman on the *published*
      // (post-buttonMap) array instead — virtualDeadmanButton is documented as
      // the published index teleop_twist_joy's enable gate reads, and setting
      // it in source space here would break under a non-identity button remap
      // (never published, or remapped onto an unrelated published slot).
      if (armed && forceMerge && dm >= 0 && dm < buttons.length) {
        buttons[dm] = 1;
      }
  ...
  useLayoutEffect(() => {
    ...
    const tick = () => {
      ...
      // The RAF polling loop (and Chrome's gamepad input delivery itself) is
      // suspended while the document is hidden, so pad snapshots freeze at
      // their last-visible values — but this interval keeps firing (throttled).
      // Publishing the frozen snapshot would repeat "deadman held + stick
      // deflected" indefinitely with the operator unable to release it.
      // Publish explicit neutral messages instead so downstream consumers see
      // all controls released while the tab is hidden.
      if (typeof document !== "undefined" && document.visibilityState === "hidden") {
        if (cfg.joyEnabled && joyTopic) {
          context.publish?.(joyTopic, {
            header: { stamp: nowStamp(), frame_id: cfg.frameId },
            axes: new Array<number>(cfg.axes).fill(0),
            buttons: new Array<number>(cfg.buttons).fill(0),
          });
        }
      ...
      if (cfg.joyEnabled && joyTopic) {
        ...
        // Synthesize the virtual deadman on the published array (see
        // applyArmedOverlay). Extend if the configured slot is past the
        // curated button count so arming is always observable downstream.
        const dm = cfg.virtualDeadmanButton;
        if (armedRef.current && dm >= 0) {
          while (buttons.length <= dm) {
            buttons.push(0);
          }
          buttons[dm] = 1;
        }
    ...
    // Background tabs throttle setInterval to >=1s (later 1/min), so the first
    // neutral publish after hiding could otherwise lag by up to a minute. Fire
    // one immediately on the hide transition.
