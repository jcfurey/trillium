...
function AnalogStick({ label, onChange }: StickProps): JSX.Element {
  ...
  const update = useCallback(
    (clientX: number, clientY: number) => {
      ...
      setPos(v);
      // Pass screen Y through unchanged: the browser Gamepad API reports
      // stick-up as a negative axis value, and negative screen Y is also up,
      // so the conventions already agree. Every other input path (physical
      // pads via useGamepadList, click-drag on the mimic SVG via
      // normalizeStick) emits that same convention — flipping here made the
      // virtual stick drive forward/back inverted relative to a physical pad
      // on the same robot.
