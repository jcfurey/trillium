...
const TWIST_STAMPED_DATATYPES = new Map([
  ["builtin_interfaces/Time", ros2jazzy["builtin_interfaces/Time"]],
  ["std_msgs/Header", ros2jazzy["std_msgs/Header"]],
  ["geometry_msgs/Vector3", ros2jazzy["geometry_msgs/Vector3"]],
  ["geometry_msgs/Twist", ros2jazzy["geometry_msgs/Twist"]],
  ["geometry_msgs/TwistStamped", ros2jazzy["geometry_msgs/TwistStamped"]],
]);
...
function TeleopStampedPanel(props: TeleopStampedPanelProps): JSX.Element {
  const { context } = props;
  ...
  const { topic: currentTopic, frameId } = config;
  useLayoutEffect(() => {
    if (!currentTopic) {
      return;
    }
    context.advertise?.(currentTopic, "geometry_msgs/TwistStamped", {
      datatypes: TWIST_STAMPED_DATATYPES,
    });
    return () => {
  ...
  useLayoutEffect(() => {
    ...
    send();
    const intervalHandle = setInterval(send, intervalMs);

    return () => {
      clearInterval(intervalHandle);
    };
  }, [context, config, currentTopic, currentAction, frameId]);
...
export default TeleopStampedPanel;
